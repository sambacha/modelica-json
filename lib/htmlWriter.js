...
<h3>Inputs</h3>
    ...
    <tr>
      ...
      <td>{{html.quantity}}</td>
      {{#hasInputConnectionType}}
      <td>{{html.connectionType}}</td>
      <td>{{html.pointType}}</td>
      {{/hasInputConnectionType}}
      <td><code>{{name}}</code><a name="{{html.anchor}}"></a></td>
      <td>{{comment}}</td>
      <td>{{html.min}}</td>
...
<h3>Protected Blocks</h3>
  ...
  var hasInputConnectionType = false
  var hasOutputConnectionType = false
  ...
  comCop.forEach(ele => {
    ...
    // Check if the connector connection type is specified
    if (ele.public && ele.public.inputs) {
      hasInputConnectionType = checkConnection(ele.public.inputs)
    }
    if (ele.public && ele.public.outputs) {
      hasOutputConnectionType = checkConnection(ele.public.outputs)
    }
    Object.assign(ele,
      { 'hasInputConnectionType': hasInputConnectionType },
      { 'hasOutputConnectionType': hasOutputConnectionType })
...
function setConnectorHTMLAttributes (connector) {
  ...
  if (connector.__cdl) {
    connector.html.connectionType = removeQuotes(connector.__cdl.connectionType)
  }
}
...
function setConnectorHTMLType (connector) {
  ...
  const type = connector.html.type
  connector.html.pointType = (type === 'Real' || type === 'Integer') ? 'Analog' : 'Binary'
  return connector
}
...
/**Return a boolean to flag if the connection type is specified
 * 
 * @param connectors A list of connectors
 * @returns boolean to flag if the connection type is specified
 */
function checkConnection (connectors) {
  var hasConnectionType = false
  for (var i=0; i<connectors.length; i++) {
    if (connectors[i].__cdl) {
